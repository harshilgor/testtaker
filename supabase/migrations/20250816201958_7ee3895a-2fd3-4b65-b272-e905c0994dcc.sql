...
-- Function to archive and reset weekly leaderboard (called by cron)
...
AS $$
BEGIN
    -- Archive current week data is already handled by the weekly table structure
    -- The new week will start fresh with new week_start_date boundaries
    -- No action needed as each week has its own records
    
    -- Log the reset for debugging
    INSERT INTO public.leaderboard_stats (user_id, display_name, total_points, visibility)
    VALUES ('00000000-0000-0000-0000-000000000000', 'SYSTEM_WEEKLY_RESET', 0, 'public')
    ON CONFLICT (user_id) DO NOTHING;
END;
$$;

-- Function to archive and reset monthly leaderboard (called by cron)
...
AS $$
BEGIN
    -- Archive current month data is already handled by the monthly table structure
    -- The new month will start fresh with new month_start_date boundaries
    -- No action needed as each month has its own records
    
    -- Log the reset for debugging
    INSERT INTO public.leaderboard_stats (user_id, display_name, total_points, visibility)
    VALUES ('00000000-0000-0000-0000-000000000001', 'SYSTEM_MONTHLY_RESET', 0, 'public')
    ON CONFLICT (user_id) DO NOTHING;
END;
$$;

-- Create cron jobs for automatic resets
...
SELECT cron.schedule(
    ...
    BEGIN
        ...
        IF CURRENT_DATE = (date_trunc('month', CURRENT_DATE) + INTERVAL '1 month - 1 day')::DATE THEN
            PERFORM public.reset_monthly_leaderboard();
        END IF;
    END $$;
    $$
);