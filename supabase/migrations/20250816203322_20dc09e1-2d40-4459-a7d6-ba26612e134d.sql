-- Fix leaderboard triggers to update periodic stats
DROP TRIGGER IF EXISTS trigger_leaderboard_on_quiz ON quiz_results;
DROP TRIGGER IF EXISTS trigger_leaderboard_on_marathon ON marathon_sessions;
DROP TRIGGER IF EXISTS trigger_leaderboard_on_mock ON mock_test_results;

-- Create new trigger function that updates both regular and periodic stats
...
-- Update periodic stats for all existing users
...
-- Trigger periodic stats update for all users with existing activity
DO $$
DECLARE
  user_record RECORD;
BEGIN
