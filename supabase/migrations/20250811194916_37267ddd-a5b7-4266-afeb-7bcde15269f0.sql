...
ALTER FUNCTION public.initialize_leaderboard_periods() SET search_path = 'public';
ALTER FUNCTION public.update_periodic_leaderboard_stats(uuid) SET search_path = 'public';
ALTER FUNCTION public.reset_weekly_leaderboard() SET search_path = 'public';
ALTER FUNCTION public.reset_monthly_leaderboard() SET search_path = 'public';
ALTER FUNCTION public.trigger_update_periodic_leaderboard() SET search_path = 'public';

-- Add additional security policy for profiles table to prevent any potential data leakage
-- This policy ensures only authenticated users can access profiles and only their own data
